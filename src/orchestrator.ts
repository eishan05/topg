...
export class Orchestrator {
  ...
  async run(userPrompt: string): Promise<OrchestratorResult> {
    ...
    while (turn < this.config.guardrailRounds) {
      ...
      if (detectConvergence(messages, turn) || (turn > 3 && checkDiffStability(messages))) {
        return this.completeWithConsensus(messages, userPrompt, meta.sessionId, turn);
      }
  ...
  async runWithHistory(
    ...
    while (turn < maxTurn) {
      ...
      if (detectConvergence(messages, segmentRounds) || (segmentRounds > 3 && checkDiffStability(messages))) {
        return this.completeWithConsensus(messages, userPrompt, sessionId, turn, signal);
      }
  ...
  async resume(sessionId: string, userGuidance?: string): Promise<OrchestratorResult> {
    ...
    if (detectConvergence(messages, turn) || (turn > 3 && checkDiffStability(messages))) {
      return this.completeWithConsensus(messages, userPrompt, sessionId, turn);
    }
  ...
  private async completeWithConsensus(
    messages: Message[],
    userPrompt: string,
    sessionId: string,
    turn: number,
    ...
    // Emit a consensus message so the dashboard displays the final answer
    const consensusMsg: Message = {
      role: "initiator",
      agent: this.agentA.name as Message["agent"],
      turn: synthTurn,
      type: "consensus",
      content: summary,
      timestamp: new Date().toISOString(),
